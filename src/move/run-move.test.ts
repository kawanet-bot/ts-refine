...
const NO_SPACE: TsSurveyReport = {bracketSpacing: {bracketSpacing: "off"}}
const SPACED: TsSurveyReport = {bracketSpacing: {bracketSpacing: "on"}}
...
describe("runMove (in-memory, dry-run)", () => {
    it("rewrites every import form and preserves the `.ts` extension where it was present", async () => {
        ...
        const result = await runMove(project, {sources: ["/src/a.ts"], dest: "/src/sub/", dryRun: true, report: NO_SPACE})
    ...
    it("does not add `.ts` to importers that omitted it", async () => {
        ...
        await runMove(project, {sources: ["/src/a.ts"], dest: "/src/sub/", dryRun: true, report: NO_SPACE})
    ...
    it("preserves the dynamic-import extension across the NodeNext js↔ts mapping (.mjs → .mts source)", async () => {
        ...
        await runMove(project, {sources: ["/src/a.mts"], dest: "/src/sub/", dryRun: true, report: NO_SPACE})
    ...
    it("treats a trailing-slash dest as a directory move when multiple sources are given", async () => {
        ...
        const result = await runMove(project, {sources: ["/src/a.ts", "/src/b.ts"], dest: "/lib/", dryRun: true, report: NO_SPACE})
    ...
    it("treats a single-source non-directory dest as a rename", async () => {
        const project = newProject()
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        const result = await runMove(project, {sources: ["/src/a.ts"], dest: "/src/renamed.ts", dryRun: true, report: NO_SPACE})
    ...
    it("rejects multi-source moves into a non-directory dest", async () => {
        ...
        await assert.rejects(
            () => runMove(project, {sources: ["/src/a.ts", "/src/b.ts"], dest: "/src/target.ts", dryRun: true, report: NO_SPACE}),
    ...
    it("rejects a source that is not in the project", async () => {
        const project = newProject()
        await assert.rejects(
            () => runMove(project, {sources: ["/src/missing.ts"], dest: "/src/sub/", dryRun: true, report: NO_SPACE}),
            /not in the project/,
        )
    })
    ...
    it("rejects when source and destination are the same path", async () => {
        ...
        project.createSourceFile("/src/a.ts", "export const x = 1\n")
        await assert.rejects(
            () => runMove(project, {sources: ["/src/a.ts"], dest: "/src/a.ts", dryRun: true, report: NO_SPACE}),
    ...
    it("rejects when destination is an existing project file that isn't being moved out", async () => {
        ...
        project.createSourceFile("/src/b.ts", "export const y = 2\n")
        await assert.rejects(
            () => runMove(project, {sources: ["/src/a.ts"], dest: "/src/b.ts", dryRun: true, report: NO_SPACE}),
    ...
    it("rejects when destination is on the project FS but excluded from the tsconfig", async () => {
        ...
        await assert.rejects(
            () => runMove(project, {sources: ["/src/a.ts"], dest: "/dist/precious.ts", dryRun: true, report: NO_SPACE}),
            /destination already exists/,
        )
    })
    ...
    it("resolves dynamic imports that target a directory's index file", async () => {
        ...
        const result = await runMove(project, {sources: ["/src/feature/index.ts"], dest: "/src/sub/index.ts", dryRun: true, report: NO_SPACE})
    ...
    it("recognizes in-memory directories without a trailing slash (project FS, not host FS)", async () => {
        ...
        const result = await runMove(project, {sources: ["/src/a.ts"], dest: "/lib", dryRun: true, report: NO_SPACE})
    ...
    it("re-sorts the import block of an import-changed file", async () => {
        ...
        await runMove(project, {sources: ["/src/m.ts"], dest: "/src/sub/", dryRun: true, report: NO_SPACE})
    ...
    it("applies the surveyed spaced style, overriding the file's no-space", async () => {
        ...
        await runMove(project, {sources: ["/src/a.ts"], dest: "/src/sub/", dryRun: true, report: SPACED})
...
describe("runMove (on disk)", () => {
    ...
    it("does not persist unrelated pending edits on the project (selective save)", async () => {
        ...
        try {
            ...
            await runMove(project, {sources: [path.join(sub, "src/a.ts")], dest: path.join(sub, "lib/"), dryRun: false, report: NO_SPACE})
    ...
    it("persists the move and the importer rewrite to disk", async () => {
        ...
        await runMove(project, {sources: [path.join(workdir, "src/a.ts")], dest: path.join(workdir, "lib/"), dryRun: false, report: NO_SPACE})
